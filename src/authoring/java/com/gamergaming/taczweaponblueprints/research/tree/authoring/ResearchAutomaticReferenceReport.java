...
/** Deterministic writer for the checked-in, appeal-free default comparison catalog. */
public final class ResearchAutomaticReferenceReport {
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

    private ResearchAutomaticReferenceReport() {
    }
    ...
    public static String toJson(List<TaCZGunStats> stats, String sourceVersion) {
        if (stats == null || stats.isEmpty()
            ...
            throw new IllegalArgumentException(
                    "Automatic weapon reference evidence cannot be empty");
        }
        if (sourceVersion == null || sourceVersion.isBlank()) {
            throw new IllegalArgumentException(
                    "Automatic weapon reference source version cannot be blank");
        }
        ...
        if (ordered.stream().map(TaCZGunStats::blueprintId).distinct().count()
                != ordered.size()) {
            throw new IllegalArgumentException(
                    "Automatic weapon reference contains duplicate blueprint IDs");
        }
        ...
        root.addProperty("format", WeaponMechanicalReferenceCatalog.CURRENT_FORMAT);
        root.addProperty(
                "reference_version",
                ResearchTechTreeContract.AUTOMATIC_REFERENCE_VERSION);
        root.addProperty(
        ...
        root.addProperty(
                "metrics_fingerprint",
                WeaponMechanicalReferenceCatalog.fingerprint(reference.distributions()));
