...
The planner computes the transitive prerequisite closure from the same
disclosure-filtered graph already published by the server. The active canvas
draws only the part of that route present in its current projection. While a
goal is active, the existing `Next` action recommends only a currently available
node inside that closure; the server remains authoritative for selection and
research.
...
If any route step is anonymous or summary-only, the UI labels the cost summary
as partial. It does not infer a hidden item, quantity, availability, identity,
or prerequisite. Datapack authors can consequently limit cost previews through
the existing Journal visibility policy without a separate planner permission.
