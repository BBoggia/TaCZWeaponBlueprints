...
This reference explains the technical evidence required for a release of
[TaCZ] Weapon Research & Blueprints. Use the shorter
[release checklist](release-checklist.md) for the reusable prepare, validate,
and publish workflow. Release commands require JDK 17.
...
- obtain a blueprint, Research Bench, and Research Data in separate fresh-player
  checks and confirm each path unlocks the same Blueprint Analyzer recipe once;
- craft the Blueprint Analyzer and confirm the Research Bench is not consumed
  or required;
- place the Blueprint Analyzer facing north, east, south, and west; confirm the paper
...
- open the dedicated Blueprint Analyzer, recycle one learned duplicate, and
